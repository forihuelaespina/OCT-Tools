...
A class for measuring class:ÌOT_RetinalLayers` thicknesses on
...
class IOT_OperationMeasureLayerThickness(IOT_Operation):
    ...
    def __init__(self):
        ...
        tmp = IOT_OperationMeasureLayerThickness() - Creates an empty MeasureLayerThickness operation
        ...
        self.layers = IOT_RetinalLayers().getAllLayersIndexes();
    ...
    def layerIndex(self,i): #layerIndex setter
        if type(i) is str: #layerName provided?
            i = IOT_RetinalLayers().getLayerIndex(i)
        self.layers = i;
        return None
    ...
    def layers(self,theLayers): #layers setter
        ...
        allLayerIndexes = IOT_RetinalLayers().getAllLayersIndexes();
        tmp=list()
        for elem in theLayers:
            if type(elem) is str: #layerName provided?
                elem = IOT_RetinalLayers().getLayerIndex(elem)
                tmp.append(elem)
    ...
    def pixelColumn(self): #pixelColumn getter
        ...
        .. note:: If py:attr:`pixelColumn` exceeds the number of columns in the
            current :class:`IOT_OCTscan` operand, then a warning is issued and 
    ...
    def execute(self,*args,**kwargs):
        ...
        if type(imgin) is IOT_OCTscanSegmentation:
            imgin=imgin.data
