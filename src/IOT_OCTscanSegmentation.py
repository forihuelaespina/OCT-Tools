...
## Class definition
class IOT_OCTscanSegmentation(object):
    #Sphinx documentation
    """A retinal layer segmentation over a :class:`IOT_OCTscan`

    A retinal layer segmentation over a :class:`IOT_OCTscan`. A segmentation 
    ...
    segmentation please refer to :class:`IOT_OperationSegmentation`.
    
    The segmentation is sized and shaped equal to its base :class:`IOT_OCTscan`.
    
    A default segmentation sets the whole segmentation to BACKGROUND.
    
    .. seealso:: :class:`IOT_OCTscan`, :class:`IOT_OperationSegmentation`
    .. note:: None
    .. todo:: None
    ...
    def __init__(self,*args):
        ...
        tmp = IOT_OCTscanSegmentation(theOCTScan) - Creates a default 
            segmentation for the given :class:`IOT_OCTscan`
        ...
        self.classMap = IOT_RetinalLayers().layers #The map of class labels
    ...
    def scan(self,octScan): #scan setter
        if octScan is not None:
            #Check whether the image is in RGB (ndim=3) or in grayscale (ndim=2)
            #and convert to grayscale if necessary
            if type(octScan) is IOT_OCTscan:
                #Dimensions are only width and height, and matches that of
                #the scan.
                self.__scan = octScan;
                self.clear()                
